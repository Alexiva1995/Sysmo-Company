                ...
                if($user->status == "1"){
                    ...
                    if(count($referidosRangoFecha) >= 22  ) {
                            ...
                            // if(count($referidosExtra) > 2){
                                if(Wallet::where([['user_id', $user->id],['bonus_id', 3]])->count() == 0){
                                    Wallet::create([
                                        'user_id' => $user->id,
                                        'bonus_id' => 3,
                                        'amount' => 400,
                                        'description' => 'Bono Speed para ' . $user->username . ' (' .$user->email . ')',
                                        'status' => 0
                                    ]);
                                    Storage::append("BonoSpeed.txt", 'Bono Speed para ' . $user->username . ' (' .$user->email . ')');
                                }
